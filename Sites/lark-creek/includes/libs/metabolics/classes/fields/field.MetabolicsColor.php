<?php
/**
 * Creates a color picker with a JS fallback
 * 
 ...
 * @since  2.0
 */
class MetabolicsColor extends MetabolicsField {

  /**
   * Creates a new color field and enqueues JS
   * @author bturley
   * @since  2.0
  ...
  /**
   * Enqueues the JS required for the fallback
   * @author bturley
   * @since  2.0
   */
  public function _enqueue() {
    wp_enqueue_script( 'jscolor', $this->core->js_path . 'jscolor/jscolor.js', array(), METABOLICS_VER, true );
  }
  ...
  public function sanitize( $post_id, $post_data = NULL ) {
    if ( isset( $post_data ) && preg_match( '/^#([a-f0-9]{3}|[a-f0-9]{6})$/i', $post_data ) )
      $value = esc_html( $post_data );
    else
      $value = NULL;
    return $value;
  }
